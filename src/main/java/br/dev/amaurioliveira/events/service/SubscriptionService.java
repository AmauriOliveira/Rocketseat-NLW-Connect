...
public class SubscriptionService {
    ...
    public SubscriptionResponse createNewSubscription(String eventName, User user, Integer userId) {
        ...
        if (temporarySubscription != null) {
            throw new SubscriptionConflictException(
                    "Já existe uma inscrição para " + recoveredUser.getName() + " no evento " + event.getTitle());
        }
        Subscription response = subscriptionRepository.save(subscription);
        return new SubscriptionResponse(response.getSubscriptionNumber(),
                "http://codecraft.com/" + response.getEvent().getPrettyName() + "/" + response.getSubscriber().getId());
    ...
    public SubscriptionRankingByUser getSubscriptionRankingByUser(String prettyName, Integer userId) {
        ...
        SubscriptionRankingItem item = rankingItems.stream().filter(i -> i.userId().equals(userId)).findFirst()
                .orElse(null);
        ...
        Integer position = IntStream.range(0, rankingItems.size())
                .filter(pos -> rankingItems.get(pos).userId().equals(userId)).findFirst().getAsInt();
